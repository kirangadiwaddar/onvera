...
async function ensureBucketExists(bucketName: string) {
  ...
  if (!isMissingBucket) {
    return { ok: false, message: getBucketError?.message || "Unable to access avatar bucket" }
  }
...
export async function POST(request: Request) {
  ...
  if (file.size > MAX_AVATAR_BYTES) {
    return NextResponse.json({ message: "Avatar must be under 300KB" }, { status: 400 })
  }

  const bucketName = process.env.SUPABASE_USER_AVATAR_BUCKET || DEFAULT_BUCKET
  ...
  const fileName = `${Date.now()}-${sanitizeFileName(file.name || "avatar.png")}`
  const filePath = `avatars/${fileName}`

  const { error: uploadError } = await admin.storage.from(bucketName).upload(filePath, file, {
    cacheControl: "3600",
    upsert: false,
